...
export type ApiResponse<T> = AdminsApiResponse<T>;
...
export function requestAdmins(callback: (response: AdminsApiResponse<Admin[]>) => void) {
  callback({
      status: 'success',
      data: admins
  });
...
export function requestCurrentServerTime(callback: (response: AdminsApiResponse<number>) => void) {
  callback({
      status: 'success',
      data: Date.now()
  });
...
export function requestCoffeeMachineQueueLength(callback: (response: AdminsApiResponse<void>) => void) {
  callback({
      status: 'error',
